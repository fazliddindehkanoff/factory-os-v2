...
test("operational duties remain separated", () => {
  ...
  assert.equal(hasPermission([role("warehouse_head")], "approvals.approve"), true)
  assert.equal(hasPermission([role("warehouse_head")], "warehouse.receive"), true)
  assert.equal(hasPermission([role("warehouse_worker")], "approvals.approve"), false)
