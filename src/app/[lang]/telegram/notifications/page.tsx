import { TelegramNotificationRead } from "@/components/telegram/telegram-notification-read"
...
export default async function Page({ params }: PageProps<"/[lang]/telegram/notifications">) {
  ...
  const session = await requireTelegramSession(lang, `/${lang}/telegram/notifications`)
  const copy = telegramCopy[lang]
  const notifications = await getTelegramNotifications(session.userId, lang)
    ...
    <TelegramShell lang={lang} copy={copy} userId={session.userId} title={copy.notifications}>
      <section aria-label={copy.notifications} className="grid grid-cols-2 gap-2.5">
        <div className="tg-card rounded-[14px] border p-3.5 shadow-[0_1px_2px_rgba(16,30,60,0.05)]">
          <span className="flex size-8 items-center justify-center rounded-[10px] bg-[#e7f1fb] text-[var(--tg-link,#2365a9)]"><BellIcon className="size-4" /></span>
          <p className="mt-3 font-mono text-[25px] font-semibold leading-none text-[var(--tg-text)]">{notifications.length}</p>
          <p className="mt-2 text-[11px] font-semibold text-[var(--tg-text-secondary)]">{uxCopy[lang].totalRecent}</p>
        </div>
        <div className="tg-card rounded-[14px] border p-3.5 shadow-[0_1px_2px_rgba(16,30,60,0.05)]">
      ...
      {unreadCount ? <div className="mb-3"><TelegramNotificationRead lang={lang} /></div> : null}
