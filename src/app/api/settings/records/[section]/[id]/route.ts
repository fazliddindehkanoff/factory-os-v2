...
export async function PATCH(
  ...
  try {
    if (["positions", "product-categories", "order-purposes", "branches"].includes(section)) {
      ...
      await databaseClient.execute({
        sql: `UPDATE products SET code = ?, category_id = ?, title_uz = ?, title_ru = ?, title_tr = ?, updated_at = CURRENT_TIMESTAMP WHERE id = ?`,
        args: [input.code!, input.categoryId!, ...titleArgs, id],
      })
